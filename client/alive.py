...
#SERVER = "https://www.98xzw.com/index/api/server"
#NODE = "6"
USER = "guest"
PASSWORD = "guest"
#TOKEN = ''
...
NODE,TOKEN,SERVER = Api.get_node()
def get_uptime():
    ...
    while True:
        if True:
            ...
            while True:
                ...
                try:
                    data = requests.post(SERVER,{'action':'heartbeat','data':json.dumps(array),'node':NODE,'token': Api.get_access(TOKEN,times), 'time':times})
                    #config()
                    #job()
                #print(data.text)
                except:
                    pass
                print('\r\033[5;31;40m监控数据透传一次{}\033[1;30;0m'.format(time.strftime("%Y-%m-%d %X",time.localtime())),end='\r')
